...
export async function analyzeGithub(owner: string, repo: string): Promise<AnalysisResult> {
  ...
  if (commits.length > 0) {
    ...
    const kwRe = /(copilot|cursor|claude|chatgpt|gpt-?[45]|gemini|generated by ai|lovable|vibe-?cod|codex|bolt\.new|\bv0\b)/i;
    const kwHits = commits.filter((c) => kwRe.test(c.commit.message));
    if (kwHits.length > 0) {
      signals.push({
        id: "ai.commit_keywords",
        category: "ai",
        label: "AI tool names in commit messages",
        weight: Math.min(24, kwHits.length * 6),
        evidence: `${kwHits.length} of ${commits.length} recent commits mention AI tools, e.g. "${kwHits[0].commit.message.split("\n")[0].slice(0, 80)}"`,
      });
    }

    const botCommits = commits.filter(
      (c) =>
        /\[bot\]|copilot|lovable|devin/i.test(c.author?.login ?? "") ||
        /\[bot\]|copilot|lovable/i.test(c.commit.author?.name ?? "") ||
        /co-authored-by:\s*(claude|copilot|cursor|lovable|chatgpt|devin)/i.test(c.commit.message),
    );
    if (botCommits.length > 0) {
      signals.push({
        id: "ai.bot_authors",
        category: "ai",
        label: "AI / bot authored commits",
        weight: Math.min(22, botCommits.length * 4),
        evidence: `${botCommits.length} commits authored or co-authored by AI tooling (e.g. ${
